...
class WorkspaceGroupsController:
    ...
    def organize_workspace_groups(
        ...
        for group_index, (group, workspaces) in enumerate(workspace_groups):
            logger.debug(
                'Organizing workspace group: "%s" in monitor "%s"', group, monitor_name
            )
            ...
            for workspace, local_number in zip(workspaces, local_numbers):
                ...
                if self.config["icons"]["enable"] and (
                    self.config["icons"]["enable_all_groups"] or group_index == 0
                ):
                    dynamic_name = self.icons_resolver.get_workspace_icons(workspace)
                    active = True
                ws_metadata.dynamic_name = dynamic_name
                new_name = ws_names.create_name(ws_metadata, active)
    ...
    def list_workspaces(
        self, group_context, focused_only: bool = False, monitor_only: bool = False
    ) -> List[i3ipc.Con]:
        workspaces = self.get_tree().workspaces()
        if monitor_only:
            workspaces = self.i3_proxy.get_monitor_workspaces()
        ...
        if not group_context:
            ...
            group_name = group_context.get_group_name(
    ...
    def _create_new_active_group_workspace_name(
        self, monitor_name: str, target_group: str
    ) -> i3ipc.Con:
        ...
        ws_metadata = ws_names.WorkspaceGroupingMetadata(
            group=target_group, global_number=global_number, local_number=local_number
        )
        return ws_names.create_name(ws_metadata, active=True)
    ...
    def switch_active_group(
        self, target_group: str, focused_monitor_only: bool
    ) -> None:
    ...
    def _create_workspace_name(
        ...
        group_index = ws_names.get_group_index(
            metadata.group, group_to_monitor_workspaces
        )
    ...
    # (name, False).
    def _get_workspace_by_local_number(
        self, group: str, local_number: int
    ) -> Tuple[str, bool]:
        ...
        return (
            self._create_workspace_name(
                ws_names.WorkspaceGroupingMetadata(
                    group=group, local_number=local_number
                )
            ),
            False,
        )
    ...
    def move_to_workspace_number(
        ...
        no_auto_back_and_forth: bool = False,
    ) -> None:
        target_workspace_name, _ = self._get_workspace_by_local_number(
            group=self._get_group_from_context(group_context),
            local_number=target_local_number,
    ...
    def _relative_workspace_in_group(self, offset_from_current: int = 1) -> i3ipc.Con:
        ...
        for (current_workspace_index, workspace) in enumerate(
            group_workspaces_all_monitors
        ):
            if workspace.id == focused_workspace.id:
                break
        next_workspace_index = (current_workspace_index + offset_from_current) % len(
            group_workspaces_all_monitors
        )
    ...
    def update_focused_workspace(
        self, metadata_updates: ws_names.WorkspaceGroupingMetadata
    ) -> None:
