...
class I3Proxy:
    ...
    def get_monitor_index(self, monitor_name):
        ordered_monitors = [
            output for output in self.i3_connection.get_outputs() if output.active
        ]
    ...
    def ps_get_monitor_name(self, con) -> str:
        try:
            while con.type != "output":
                con = con.parent
            return con.name
        except:
            return self.get_focused_monitor_name()

    def ps_find_workspace_by_name(self, name):
        workspaces = self.i3_connection.get_workspaces()
    ...
    def focus_workspace(self, name: str, auto_back_and_forth: bool = True) -> None:
        ...
        # self.send_i3_command(f'''workspace {options} "{name.replace('"', r'\"')}"''')
        self.send_i3_command(
            '''workspace {0} "{1}"'''.format(options, name.replace('"', r"\""))
        )
    ...
    def rename_workspace(self, old_name: str, new_name: str) -> None:
        if old_name == new_name:
            return
        self.send_i3_command(
            # f'''rename workspace "{old_name.replace('"', r'\"')}" to "{new_name}"'''
            '''rename workspace "{0}" to "{1}"'''.format(
                old_name.replace('"', r"\""), new_name
            )
        )
    ...
    def get_unique_marked_workspace(self, mark) -> Optional[i3ipc.Con]:
        ...
        if len(workspaces) > 1:
            logger.warning(
                "Multiple workspaces marked with %s, using first " "one", mark
            )
        return workspaces[0]
